...
{{-- Built by Bagas (Baprade) - Day 3: Interactive Split Workspace, Google SSO Modal & Test Runner UI - 28 Jul 2026 --}}
...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8" x-data="{
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        {{-- Left Pane: Problem Statement & Instructions --}}
        <div class="lg:col-span-5 natural-card p-6 rounded-2xl border border-slate-800 space-y-6">
            <div class="space-y-2">
                <div class="flex items-center gap-2">
                    ...
                    <span class="text-[10px] font-bold px-2 py-0.5 rounded {{ $challenge['difficulty'] === 'Easy' ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/30' : 'bg-amber-500/10 text-amber-400 border border-amber-500/30' }}">
            ...
            <div class="space-y-2 border-t border-slate-800/80 pt-4">
                ...
                <div class="space-y-2 text-xs font-mono">
                    @foreach($challenge['test_cases'] as $idx => $tc)
                    <div class="p-3 rounded bg-slate-900/80 border border-slate-800/80 space-y-1">
                        <div class="text-[10px] text-slate-400 uppercase font-bold">Case #{{ $idx + 1 }}</div>
                        <div class="text-slate-300">Input: <span class="text-emerald-400">{{ json_encode($tc['input']) }}</span></div>
                        <div class="text-slate-300">Expected: <span class="text-emerald-400">{{ json_encode($tc['expected']) }}</span></div>
                    </div>
                    @endforeach
                </div>
            </div>
                ...
                <div x-show="showHint" x-collapse class="p-3 rounded bg-amber-950/20 border border-amber-500/30 text-amber-300 text-xs space-y-1">
                    ...
                    <ul class="list-disc list-inside space-y-1">
                        ...
                        <template x-if="lang === 'en'">
                            <div>
                                @foreach($challenge['hints_en'] as $hint)
                                <li>{{ $hint }}</li>
                                @endforeach
                            </div>
                        </template>
    ...
    <div x-show="showAuthModal" x-transition.opacity class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md">
        <div @click.away="showAuthModal = false" class="natural-card max-w-md w-full p-6 rounded-2xl border border-slate-800 bg-[#090d16] space-y-5 text-center shadow-2xl relative">
            ...
            <div class="space-y-2.5 pt-2">
                <a href="{{ route('auth.google') }}" class="w-full inline-flex items-center justify-center gap-2 p-3 rounded-xl bg-white text-slate-900 hover:bg-slate-100 font-bold text-xs transition-all shadow-md">
                ...
                <a href="{{ route('auth.demo') }}" class="w-full inline-flex items-center justify-center gap-1.5 p-2.5 rounded-xl bg-slate-900 border border-slate-800 text-slate-300 hover:text-white hover:bg-slate-800 text-xs font-semibold transition-all">
