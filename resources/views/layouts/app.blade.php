...
<head>
    ...
    {{-- SEO Core Meta Tags --}}
    <title>{{ $seo['title'] ?? config('learn_seo.title') }}</title>
    <meta name="description" content="{{ $seo['description'] ?? config('learn_seo.description') }}">
    <meta name="keywords" content="{{ $seo['keywords'] ?? config('learn_seo.keywords') }}">
    <meta name="author" content="{{ $seo['author'] ?? config('learn_seo.author') }}">
    <meta name="robots" content="index, follow, max-image-preview:large, max-snippet:-1, max-video-preview:-1">
    <link rel="canonical" href="{{ $seo['canonical_url'] ?? config('learn_seo.canonical_url') }}">

    {{-- Open Graph / Facebook / WhatsApp --}}
    <meta property="og:type" content="website">
    <meta property="og:site_name" content="{{ $seo['site_name'] ?? config('learn_seo.site_name') }}">
    <meta property="og:title" content="{{ $seo['title'] ?? config('learn_seo.title') }}">
    <meta property="og:description" content="{{ $seo['description'] ?? config('learn_seo.description') }}">
    <meta property="og:url" content="{{ $seo['canonical_url'] ?? config('learn_seo.canonical_url') }}">
    <meta property="og:image" content="{{ $seo['og_image'] ?? config('learn_seo.og_image') }}">
    <meta property="og:locale" content="id_ID">

    {{-- Twitter Card --}}
    <meta name="twitter:card" content="summary_large_image">
    <meta name="twitter:title" content="{{ $seo['title'] ?? config('learn_seo.title') }}">
    <meta name="twitter:description" content="{{ $seo['description'] ?? config('learn_seo.description') }}">
    <meta name="twitter:image" content="{{ $seo['og_image'] ?? config('learn_seo.og_image') }}">
...
class="bg-[#090d16] text-slate-200 antialiased selection:bg-emerald-500 selection:text-white relative min-h-screen flex flex-col justify-between">
    ...
    {{-- Dynamic Interactive Mouse & Touch Ambient Canvas Background --}}
    ...
    <header class="fixed top-0 left-0 right-0 z-50 bg-[#090d16]/90 backdrop-blur-md border-b border-slate-800 shadow-lg">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex items-center justify-between h-16">
                {{-- Clean Brand Logo with favico.ico --}}
                <a href="{{ route('home') }}" class="flex items-center gap-2.5 group">
                    <img src="{{ asset('favico.ico') }}" alt="Learn Baprade Logo" class="w-8 h-8 rounded object-contain bg-slate-900 border border-slate-800 p-0.5 shadow-sm group-hover:border-emerald-500/50 transition-all">
                ...
                <nav class="hidden md:flex items-center gap-6 text-xs font-medium text-slate-300">
                    <a href="{{ route('home') }}" class="hover:text-emerald-400 transition-colors">
                        <span x-show="lang === 'id'">Katalog Tantangan</span>
                        <span x-show="lang === 'en'">Challenge Catalog</span>
                    </a>
                    <a href="{{ config('learn_seo.jasa_site') }}" target="_blank" rel="noopener" class="hover:text-emerald-400 transition-colors flex items-center gap-1">
                        <span x-show="lang === 'id'">Jasa Web & App</span>
                        <span x-show="lang === 'en'">Web Dev Services</span>
                    </a>
                    <a href="{{ config('learn_seo.personal_site') }}" target="_blank" rel="noopener" class="text-slate-400 hover:text-white transition-colors flex items-center gap-1">
                ...
                <div class="flex items-center gap-3">
                    ...
                    <div x-data="{ userMenu: false }" class="relative">
                        <button type="button" @click="userMenu = !userMenu" class="flex items-center gap-2 p-1 rounded-lg bg-slate-900 border border-slate-800 hover:border-emerald-500/50 transition-all">
                            <img src="{{ auth()->user()->avatar ?? 'https://ui-avatars.com/api/?name='.urlencode(auth()->user()->name).'&background=10b981&color=fff' }}" alt="{{ auth()->user()->name }}" class="w-6 h-6 rounded-full object-cover">
                            <span class="text-xs font-semibold text-white hidden sm:inline-block max-w-[100px] truncate">{{ auth()->user()->name }}</span>
                        ...
                        <div x-show="userMenu" @click.away="userMenu = false" x-collapse class="absolute right-0 mt-2 w-48 rounded-xl bg-slate-900 border border-slate-800 shadow-xl p-2 z-50 text-xs space-y-1">
                            <div class="px-3 py-1.5 border-b border-slate-800 text-slate-400">
                                ...
                                <div class="text-[10px] truncate">{{ auth()->user()->email }}</div>
                            </div>
                    ...
                    @else
                    <a href="{{ route('auth.google') }}" class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded bg-white text-slate-900 hover:bg-slate-100 text-xs font-bold transition-all shadow-sm">
    ...
    <footer class="bg-slate-950 border-t border-slate-900 py-6 relative z-10 text-xs text-slate-400">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col sm:flex-row items-center justify-between gap-3">
            <div class="flex items-center gap-2">
                <img src="{{ asset('favico.ico') }}" alt="Learn Baprade Logo" class="w-5 h-5 rounded object-contain">
                <span>&copy; {{ date('Y') }} Learn Baprade by Bagas. Built with Laravel 13 & SOLID Architecture.</span>
            </div>
            <div class="flex items-center gap-4 text-slate-400">
                <a href="{{ config('learn_seo.personal_site') }}" target="_blank" class="hover:text-white transition-colors">baprade.my.id</a>
                <a href="{{ config('learn_seo.jasa_site') }}" target="_blank" class="hover:text-white transition-colors">jasa.baprade.my.id</a>
            </div>
        </div>
    </footer>

    {{-- Canvas Interactive Particle Script --}}
    <script>
        (function() {
