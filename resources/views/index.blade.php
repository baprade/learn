...
{{-- Built by Bagas (Baprade) - Day 3: Challenge Listing Dashboard & Progress Tracker - 28 Jul 2026 --}}
...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-10" x-data="{ 
    ...
    <div class="text-center max-w-3xl mx-auto space-y-4 pt-6">
        ...
        <div class="p-4 rounded-xl bg-slate-900/90 border border-slate-800 max-w-md mx-auto flex items-center justify-between gap-4 text-xs">
            <div class="flex items-center gap-2">
                ...
                <div class="text-left">
                    ...
                    <div class="text-slate-400 text-[11px]">
                        <span x-text="solvedCount"></span> / {{ count($challenges) }} 
            ...
            <div class="w-32 bg-slate-800 h-2 rounded-full overflow-hidden">
                <div class="bg-emerald-400 h-full transition-all duration-500" :style="'width: ' + Math.min(100, Math.round((solvedCount / {{ count($challenges) }}) * 100)) + '%'"></div>
            </div>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <div x-show="filterCategory === 'All' || filterCategory === '{{ $ch['category'] }}'" class="natural-card p-6 rounded-2xl border border-slate-800 flex flex-col justify-between space-y-4 hover:border-emerald-500/50 transition-all group">
            ...
            <div class="space-y-3">
                <div class="flex items-center justify-between gap-2">
                    ...
                    <div class="flex items-center gap-1.5">
                        <span class="text-[10px] font-bold px-2 py-0.5 rounded {{ $ch['difficulty'] === 'Easy' ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/30' : 'bg-amber-500/10 text-amber-400 border border-amber-500/30' }}">
                            {{ $ch['difficulty'] }}
                        </span>
                        <span class="text-[10px] font-semibold text-slate-400 font-mono">+{{ $ch['points'] }} pts</span>
                    </div>
                </div>

                <h2 class="text-base font-bold font-heading text-white group-hover:text-emerald-400 transition-colors">
                    <span x-show="lang === 'id'">{{ $ch['title_id'] }}</span>
                    <span x-show="lang === 'en'">{{ $ch['title_en'] }}</span>
                </h2>
            ...
            <div class="pt-3 border-t border-slate-800/80 flex items-center justify-between">
                <div class="flex items-center gap-1 text-[11px]" :class="solvedSlugs.includes('{{ $ch['slug'] }}') ? 'text-emerald-400 font-semibold' : 'text-slate-500'">
                    <template x-if="solvedSlugs.includes('{{ $ch['slug'] }}')">
                ...
                <a href="{{ route('challenge.show', $ch['slug']) }}" class="inline-flex items-center px-4 py-2 rounded border border-emerald-500/80 text-xs font-semibold text-white bg-emerald-600 hover:bg-emerald-500 transition-all">
