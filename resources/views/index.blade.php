...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8" x-data="{ 
    ...
    <div class="max-w-3xl mx-auto text-center space-y-3 pt-4">
        ...
        <div class="p-3.5 rounded-xl bg-slate-900/90 border border-slate-800 max-w-md mx-auto flex items-center justify-between gap-4 text-xs">
            ...
            <div class="w-28 bg-slate-800 h-2 rounded-full overflow-hidden">
                <div class="bg-emerald-400 h-full transition-all duration-300" :style="'width: ' + Math.min(100, Math.round((solvedCount / {{ count($challenges) }}) * 100)) + '%'"></div>
            </div>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
        ...
        <div x-show="(track === 'all' || track === '{{ $ch['language'] ?? 'php' }}') && (filterCategory === 'All' || filterCategory === '{{ $ch['category'] }}')" 
            ...
            <div class="space-y-2.5">
                <div class="flex items-center justify-between gap-2">
                    ...
                    <div class="flex items-center gap-1.5">
                        ...
                        <span class="text-[10px] text-slate-400 font-mono">+{{ $ch['points'] }} pts</span>
                    </div>
                </div>
            ...
            <div class="pt-3 border-t border-slate-800/80 flex items-center justify-between">
                <div class="text-[11px]" :class="solvedSlugs.includes('{{ $ch['slug'] }}') ? 'text-emerald-400 font-semibold' : 'text-slate-500'">
                    <span x-show="solvedSlugs.includes('{{ $ch['slug'] }}')" class="inline-flex items-center gap-1">
                        <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path></svg>
                        <span>Solved</span>
                    </span>
                    <span x-show="!solvedSlugs.includes('{{ $ch['slug'] }}')">Unsolved</span>
                </div>

                <a href="{{ route('challenge.show', $ch['slug']) }}" class="inline-flex items-center px-3.5 py-1.5 rounded border border-emerald-500/80 text-xs font-semibold text-white bg-emerald-600 hover:bg-emerald-500 transition-all">
